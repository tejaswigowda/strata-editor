...
| argument extraction + host-normalizer | ✅ | **Normalizer EXISTS** (work order asked to confirm): `editOps.js` `recolorOp` does `new window.THREE.Color(color).getHex()` — accepts `'black'`, `'red'`, `'#111'`, `0xff0000` and resolves to a numeric hex once; the inline comment explicitly calls this the "`black`→#111 scaffolding for the model." `colorName.js` is the *reverse* map (rgb→name) used by `classDerive` color classes. |
...
| Scale / Y-up export | ✅ | three.js and glTF are both Y-up, metre-scale; `GLTFExporter` preserves node transforms — no conversion needed, scale exports faithfully. (Import normalization is separate: `import/normalize.js`.) |
