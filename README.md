...
| **The call card** | A living end-card (`/about/callcard`) any short can embed via **Stencils → Call card** — tagline, mark, QR to the editor, version stamp. Live views always show the current card; renders bake a fresh snapshot at render time, so updating the card once updates every future render without touching a single scene. The scene stores a reference, never a copy. See [CALLCARD.md](guides/CALLCARD.md). |
