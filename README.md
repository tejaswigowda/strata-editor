...
| [**Animation**](guides/ANIMATION.md) | The scene-wide universal timeline, authored with the ONE grammar: jQuery `.animate()` over CSS 3D transforms + `.at()` absolute placement, CSS easing, look-at baking, entrance/exit/attention recipes, lifecycle. A keyframe P/S/R editor in the Animations tab keys poses at the playhead (rotation deferrable to look-at). The Render tab exports the timeline as video through a single camera or a multi-shot camera sequence with cut/fade transitions, with an optional subtitle track (hard-burned or exported as a sidecar `.srt`). |
...
| [**The call card**](guides/CALLCARD.md) | The `/about` page and the living `/about/callcard` end-card every short can embed — living vs. baked, the trust boundary, and how to add it to a scene. |
...
| **Git versioning** | Auto-load, commit, split-screen merge-conflict resolution. AI writes diff-aware messages. Diffable JSON. Reads (scene + assets) resolve through CDN edges (jsDelivr, raw.githubusercontent.com) rather than the GitHub API, so a shared classroom IP or a widely-shared link never trips GitHub's anonymous rate limit — the API is only a last-resort fallback, never required to view a public repo. **Shareable links:** `#repo=owner/repo[@ref]&file=path.json[&branch=name]` in the URL preloads that scene straight from a git repo on page load (a token is only ever needed to commit). See [GIT_VERSIONING.md](guides/GIT_VERSIONING.md). |
...
| **Video render** | Render tab: export the universal timeline as video (mp4/webm, up to 1080p, 24/30/60 fps) through any camera — or a **camera sequence**: draggable shot blocks with cut/crossfade transitions, versioned with the scene. Built-in **subtitle editor**: add/import/export timed caption cues, hard-burn them into the exported frames, or download a matching `.srt` sidecar for downstream muxing. Every export carries a silent audio track (so it never breaks a downstream `ffmpeg -map 0:a`). Progress bar, live preview, one-click download. |
| **The call card** | A living end-card (`/about/callcard`) any short can embed via **Stencils → Call card** — tagline, mark, QR to the editor, version stamp. Live views always show the current card; renders bake a fresh snapshot at render time, so updating the card once updates every future render without touching a single scene. The scene stores a reference, never a copy. Reveal it with a cinematic push-in that ends with the card filling the whole frame — see [CALLCARD.md](guides/CALLCARD.md). |
...
- **Done.** Deterministic language (selectors + ops), `$S()` API, universal timeline (absolute keyframes + `.animate()`/`.at()` sugar), git versioning, scene intelligence, constrained decoding, **eval matrix** (1.5B is viable floor), multi-op segmentation, bulk property ops, standalone [3DOM library](https://github.com/tejaswigowda/3dom) (Strata consumes via host adapter), host-side selector + op-type resolution (parity by construction, 22/22 fixture agreement, clean Haiku re-run).
