...
| [**Animation**](guides/ANIMATION.md) | The scene-wide universal timeline, authored with the ONE grammar: jQuery `.animate()` over CSS 3D transforms + `.at()` absolute placement, CSS easing, look-at baking, entrance/exit/attention recipes, lifecycle. The Render tab exports the timeline as video through a single camera or a multi-shot camera sequence with cut/fade transitions. |
